    ...
    class Vsphere
      class Real
        def update_vm(server)
          ...
          # Volumes
          # TODO: See clone_vm for an example
        ...
        def update_vm_interfaces_specs(vm_mob_ref, fog_interfaces, datacenter)
          ...
          vm_nics.zip(modified_nics).each do |vm_nic, fog_nic|
            if fog_nic
              # updated the attributes on the existing nic
          ...
          #specs.concat(new_nics.map { |nic| create_interface(nic, 0, :add, datacenter: datacenter) })
          specs
        end

        def nic_backing_different?(one, two)
          one.class != two.class ||
            (one.is_a?(RbVmomi::VIM::VirtualEthernetCardDistributedVirtualPortBackingInfo) && (one.port.portgroupKey != two.port.portgroupKey || one.port.switchUuid != two.port.switchUuid)) || (one.is_a?(RbVmomi::VIM::VirtualEthernetCardNetworkBackingInfo) && one.deviceName != two.deviceName)
        end
      end
